...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    require$$1$3.app.setAsDefaultProtocolClient(PROTOCOL, process.execPath, [require$$1$1.resolve(process.argv[1])]);
  }
} else {
...
function createFlowWindow() {
  ...
  const win = new require$$1$3.BrowserWindow({
    ...
    webPreferences: {
      preload: require$$1$1.join(__dirname, "index.js"),
...
function showFlow() {
  ...
  const cursor = require$$1$3.screen.getCursorScreenPoint();
  const display = require$$1$3.screen.getDisplayNearestPoint(cursor);
...
require$$1$3.ipcMain.on("flow:hide", hideFlow);
require$$1$3.ipcMain.on("flow:mouseEnter", () => {
  if (!flowWindow) return;
  flowWindow.setIgnoreMouseEvents(false);
});
require$$1$3.ipcMain.on("flow:mouseLeave", () => {
  if (!flowWindow) return;
  flowWindow.setIgnoreMouseEvents(true, { forward: true });
});
require$$1$3.ipcMain.on("shell:openExternal", (_e, url) => {
  require$$1$3.shell.openExternal(url);
});
require$$1$3.ipcMain.handle("theme:get", () => require$$1$3.nativeTheme.shouldUseDarkColors);
require$$1$3.ipcMain.on("settings:setTheme", (_e, theme) => {
  if (flowWindow) {
...
require$$1$3.ipcMain.on("settings:open", toggleSettings);
...
require$$1$3.ipcMain.on("auth:login", () => {
  const apiUrl = process.env.NAVI_API_URL || "https://navi-search.vercel.app";
  require$$1$3.shell.openExternal(`${apiUrl}/api/auth/login`);
});
require$$1$3.ipcMain.on("auth:logout", () => {
  if (flowWindow) {
...
require$$1$3.app.whenReady().then(() => {
  ...
  if (!isMac) {
    require$$1$3.nativeTheme.on("updated", () => {
      if (tray) {
...
require$$1$3.app.on("will-quit", () => require$$1$3.globalShortcut.unregisterAll());
